...
one_hot_encoding_cooking = {"seldoms": [
    1, 0, 0], "sometimes": [0, 1, 0], "always": [0, 0, 1]}
one_hot_encoding_wake_time = {"before_6": [
    1, 0, 0], "6_to_9": [0, 1, 0], "after_9": [0, 0, 1]}
one_hot_encoding_sleep_time = {"before_10": [
    1, 0, 0], "10_to_12": [0, 1, 0], "after_12": [0, 0, 1]}
one_hot_encoding_gender = {"male": [1, 0, 0],
                           "female": [0, 1, 0], "non-binary": [0, 0, 1]}
one_hot_encoding_career = {"student": [1, 0, 0, 0, 0, 0, 0, 0, 0], "worker": [0, 1, 0, 0, 0, 0, 0, 0, 0], "retiree": [0, 0, 1, 0, 0, 0, 0, 0, 0], "doctor": [0, 0, 0, 1, 0, 0, 0, 0, 0],
                           "engineer": [0, 0, 0, 0, 1, 0, 0, 0, 0], "teacher": [0, 0, 0, 0, 0, 1, 0, 0, 0], "designer": [0, 0, 0, 0, 0, 0, 1, 0, 0], "artist": [0, 0, 0, 0, 0, 0, 0, 1, 0], "other": [0, 0, 0, 0, 0, 0, 0, 0, 1]}

qtf_partner = {"yes": 0, "no": 1}
...
qty_stay_with_landlord = {"yes": 0, "no": 1}
qtf_park = {"yes": 0, "no": 1}
qty_pet = {"yes": 0, "no": 1}
qtf_smoking = {"yes": 0, "no": 1}
...
def transform_one_user(user):
    ...
    qtf_dict = {
        ...
        "gender": one_hot_encoding_gender[user["basic_info"]["gender"]],
        "identity": one_hot_encoding_career[user["basic_info"]["identity"]],
        "partner": qtf_partner[user["basic_info"]["partner"]],
        "introduction": user["basic_info"]["introduction"],
        "price": int(user["house_preference"]["price"]),
        "house_age": int(user["house_preference"]["house_age"]),
        "stay_with_landlord": qty_stay_with_landlord[user["house_preference"]
                                                     ["stay_with_landlord"]],
        "park_nearby": qtf_park[user["house_preference"]["park_nearby"]],
        "sleep_time": one_hot_encoding_sleep_time[get_sleep_time_category(user["routine"]["sleepTime"])],
        "wake_time": one_hot_encoding_wake_time[get_wake_time_category(user["routine"]["wakeUpTime"])],
        "hygiene_tolerance": user["routine"]["hygieneTolerance"],
        "noise_tolerance": user["routine"]["noiseTolerance"],
        "cook_options": one_hot_encoding_cooking[user["routine"]["cookOptions"]],
        "pet_options": qty_pet[user["routine"]["petOptions"]],
        "smoke_options": qtf_smoking[user["routine"]["smokeOptions"]]
    }
